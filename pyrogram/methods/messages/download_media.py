...
def purchased_paid_media(paid_media_info: "types.PaidMediaInfo") -> Optional[list]:
...
def save_inline_thumbnail(
    ...
    message: Union["types.StrippedThumbnail", "types.PaidMediaPreview"],
    file_name: str,
    in_memory: bool
) -> Union[str, BinaryIO]:
    ...
    file_path = os.path.abspath(re.sub("\\\\", "/", os.path.join(directory, name)))
...
async def download_paid_media(
    ...
    progress_args: tuple
) -> Optional[List[Union[str, BinaryIO]]]:
    directory, name = os.path.split(file_name)
    results = []
    ...
    for index, item in enumerate(media):
        ...
        if name and len(media) > 1:
            ...
            item_name = "{}_{}{}".format(stem, index + 1, extension)
